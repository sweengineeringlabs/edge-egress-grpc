//! `ResilientAssembler` — builds a resilient gRPC transport stack.
...
/// Assembles a resilient gRPC transport from a channel configuration.
pub(crate) struct ResilientAssembler;

impl ResilientAssembler {
    /// Build a `GrpcEgress` transport from `config`, optionally wrapped in
    /// retry + circuit-breaker layers when resilience is configured.
