...
impl ResilienceConfigBuilder {
    ...
    pub fn max_attempts(mut self, v: u32) -> Self { self.max_attempts = Some(v); self }
    pub fn initial_backoff_ms(mut self, v: u64) -> Self { self.initial_backoff_ms = Some(v); self }
    pub fn backoff_multiplier(mut self, v: f64) -> Self { self.backoff_multiplier = Some(v); self }
    pub fn jitter_factor(mut self, v: f64) -> Self { self.jitter_factor = Some(v); self }
    pub fn max_backoff_ms(mut self, v: u64) -> Self { self.max_backoff_ms = Some(v); self }
    pub fn rate_limit_max_attempts(mut self, v: u32) -> Self { self.rate_limit_max_attempts = Some(v); self }
    pub fn rate_limit_initial_backoff_ms(mut self, v: u64) -> Self { self.rate_limit_initial_backoff_ms = Some(v); self }
    pub fn rate_limit_max_backoff_ms(mut self, v: u64) -> Self { self.rate_limit_max_backoff_ms = Some(v); self }
    pub fn failure_threshold(mut self, v: u32) -> Self { self.failure_threshold = Some(v); self }
    pub fn cool_down_seconds(mut self, v: u64) -> Self { self.cool_down_seconds = Some(v); self }
    pub fn half_open_probe_count(mut self, v: u32) -> Self { self.half_open_probe_count = Some(v); self }
    ...
    pub fn build(self) -> Result<ResilienceConfig, String> {
        Ok(ResilienceConfig {
            ...
            rate_limit_max_attempts:       self.rate_limit_max_attempts.ok_or("rate_limit_max_attempts required")?,
...
mod tests {
    ...
    fn test_rate_limit_initial_backoff_ms_sets_field() {
        let c = full_builder().rate_limit_initial_backoff_ms(500).build().unwrap();
    ...
    fn test_rate_limit_max_backoff_ms_sets_field() {
        let c = full_builder().rate_limit_max_backoff_ms(20_000).build().unwrap();
    ...
    fn test_build_missing_required_field_returns_err() {
        let r = ResilienceConfigBuilder::new().rate_limit_max_attempts(2).build();
        assert!(r.is_err());
    }
}
