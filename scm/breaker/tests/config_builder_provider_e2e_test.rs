...
use swe_edge_egress_grpc_breaker::{
    ConfigBuilderProvider, ConfigBuilderRequest, ConfigBuilderResponse, Error, GrpcBreakerSvc,
};
...
/// provider) without being a re-test of `GrpcBreakerSvc` itself.
...
impl ConfigBuilderProvider for DelegatingConfigBuilderProvider {
    fn create_config_builder(
        ...
        GrpcBreakerSvc.create_config_builder(req)
    }
}
...
fn test_default_provider_is_deterministic_edge() {
    let a = <GrpcBreakerSvc as ConfigBuilderProvider>::default_provider();
    let b = <GrpcBreakerSvc as ConfigBuilderProvider>::default_provider();
    assert_eq!(
