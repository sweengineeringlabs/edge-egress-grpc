//! `impl ConfigBuilderProvider for GrpcBreakerSvc`.

use tracing::debug;

use crate::api::{
    ApplicationConfigBuilder, BreakerDomainError, ConfigBuilderProvider, ConfigBuilderRequest,
    ConfigBuilderResponse, GrpcBreakerSvc, CONFIG_BUILDER_PROVIDER_SECTION,
};
...
impl ConfigBuilderProvider for GrpcBreakerSvc {
    fn create_config_builder(
        ...
        debug!(
            section = CONFIG_BUILDER_PROVIDER_SECTION,
            "grpc-breaker: pre-seeding config builder",
        );
    ...
    fn test_create_config_builder_build_loader_produces_a_working_loader() {
        ...
        let err = loader
            .load_section::<ConfigBuilderProviderAbsentSectionProbe>(
                "breaker_core_probe_section_that_does_not_exist",
            )
            .expect_err("no config directory exists in the test environment");
        assert!(err
            .to_string()
            .contains("breaker_core_probe_section_that_does_not_exist"));
    }

    #[test]
    fn test_default_provider_returns_a_grpc_breaker_svc_marker() {
        let svc = <GrpcBreakerSvc as ConfigBuilderProvider>::default_provider();
